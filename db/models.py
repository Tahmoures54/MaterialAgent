...
class Product(Base):
    ...
    def __repr__(self):
        return f"<Product(item_code='{self.item_code}')>"
...
class Transaction(Base):
    ...
    def __repr__(self):
        return f"<Transaction(id={self.id}, item='{self.item_code}')>"
...
class Stock(Base):
    ...
    qc_status = Column(String(20), nullable=False, default="QUARANTINE", index=True)
...
class MaterialRequest(Base):
    ...
    request_type = Column(String(20), default="Normal")
    iso_drawing_no = Column(String(100), nullable=True)
...
class MaterialRequestLine(Base):
    ...
    def remaining_qty(self):
        return max(0.0, float(self.qty or 0) - float(self.fulfilled_qty or 0))
