...
from db.models import (
    ...
    Stock, Location, Document
)


def generate_inventory_summary() -> List[Dict]:
    """
    ...
    try:
        ...
        ).outerjoin(Stock, Product.item_code == Stock.item_code)\
...
def get_low_stock_items(threshold: float = 10) -> List[Dict]:
    """
    Get items with available stock below threshold.
    Uses Stock model for accurate data.
    """
    session = get_db_session()
    try:
        results = session.query(
        ...
        for row in results:
            data.append({
                ...
                'Shortage': max(0, (row.min_required_qty or 0) - row.available_qty)
            })
        return data
    finally:
        session.close()
...
def get_stock_value_report() -> List[Dict]:
    """
    Calculate stock value report.
    Requires unit_cost in Product model.
    """
    session = get_db_session()
    try:
        results = session.query(
        ...
        data.append({
            ...
            'Unit Cost': '',
            'Total Qty': sum(d['Total Qty'] for d in data),
...
def get_movement_report(
    ...
    try:
        # Get document lines
        from db.models import DocumentLine
